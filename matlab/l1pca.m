...
function [Qopt, Bopt,U,V]=l1pca(X, K)
...
[U, S, ~]=svd(X);
s=diag(S);
d=sum(s>toler); %sommo il numero di autovalori non nulli

%%condizione per cui non si possono ottenere più autovettori destri di
%%quanti autovalori non nulli siano presenti
if K>d
    Qopt = []; Bopt = []; U = []; V = [];
    return;
